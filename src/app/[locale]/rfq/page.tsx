  ...
  return (
    <div className="min-h-screen dark:bg-[#0d1410] bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <h1 className="mb-6 text-2xl font-bold dark:text-gray-100 text-slate-900">{t('title')}</h1>
        ...
        <div className="mb-8 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-4 shadow-sm">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
            <div>
              <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">{t('commodity')}</label>
            ...
            <div>
              <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">{t('sortBy')}</label>
        ...
        {loading ? (
          ...
          <>
            <p className="mb-4 text-sm dark:text-[#8b9e93] text-slate-500">{t('showingRfqs', { count: rfqs.length, total })}</p>
                ...
                <Link key={r.id} href={`/rfq/${r.id}`}
                  ...
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    {r.status === 'open' && new Date(r.neededBy) > new Date() && (
                      <span className="rounded dark:bg-orange-500/15 dark:text-orange-400 bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-700">{t('neededBy')} {formatDate(r.neededBy)}</span>
                    )}
                  </div>
