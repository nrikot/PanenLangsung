...
export default function RfqDetailPage() {
  ...
  async function submitQuote(e: React.FormEvent) {
    ...
    try {
      ...
      setQuoteMsg(t('quoteSubmitted'));
      setQuoteForm({ pricePerUnit: '', quantity: '', message: '' });
      fetchRfq();
    } catch (e: unknown) { setQuoteMsg(e instanceof Error ? e.message : t('quoteFailed')); }
  ...
  return (
    <div className="min-h-screen dark:bg-[#0d1410] bg-slate-50">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <Link href="/rfq" className="mb-4 inline-block text-sm dark:text-green-400 text-green-600 hover:underline">&larr; {t('backToRfq')}</Link>
        ...
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-6 shadow-sm">
          ...
          <h1 className="mb-4 text-xl font-bold dark:text-gray-100 text-slate-900">{t('requestTitle', { name: rfq.commodity.name })}</h1>

          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-lg dark:bg-white/5 bg-slate-50 p-3">
              <p className="text-xs dark:text-[#8b9e93] text-slate-500">{t('quantity')}</p>
              <p className="text-lg font-bold dark:text-gray-100 text-slate-900">{rfq.quantity.toLocaleString('id-ID')} {rfq.unit}</p>
            </div>
            <div className="rounded-lg dark:bg-white/5 bg-slate-50 p-3">
              <p className="text-xs dark:text-[#8b9e93] text-slate-500">{t('targetPrice')}</p>
              <p className="text-lg font-bold dark:text-gray-100 text-slate-900">{rfq.targetPrice ? formatRupiah(rfq.targetPrice) : '-'}</p>
            </div>
            <div className="rounded-lg dark:bg-white/5 bg-slate-50 p-3">
              <p className="text-xs dark:text-[#8b9e93] text-slate-500">{t('neededBy')}</p>
              <p className="text-lg font-bold dark:text-gray-100 text-slate-900">{formatDate(rfq.neededBy)}</p>
            </div>
            <div className="rounded-lg dark:bg-white/5 bg-slate-50 p-3">
              <p className="text-xs dark:text-[#8b9e93] text-slate-500">{t('bids')}</p>
              <p className="text-lg font-bold dark:text-gray-100 text-slate-900">{rfq.quotes.length}</p>
            </div>
          </div>

          <div className="mb-6 space-y-2 text-sm">
            <div className="flex justify-between"><span className="dark:text-[#8b9e93] text-slate-500">{t('buyer')}</span><span className="dark:text-gray-100 text-slate-900">{rfq.buyer.businessName || rfq.buyer.name}</span></div>
            <div className="flex justify-between"><span className="dark:text-[#8b9e93] text-slate-500">{t('deliveryLocation')}</span><span className="dark:text-gray-100 text-slate-900 text-right max-w-[60%]">{rfq.deliveryLocation}</span></div>
            <div className="flex justify-between"><span className="dark:text-[#8b9e93] text-slate-500">{t('created')}</span><span className="dark:text-gray-100 text-slate-900">{formatDateTime(rfq.createdAt)}</span></div>
          </div>
        </div>

        {canQuote && (
          <div className="mt-6 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold dark:text-gray-100 text-slate-900">{myQuote ? t('updateQuote') : t('submitQuote')}</h2>
            <form onSubmit={submitQuote} className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">{t('quotePrice')}</label>
                  <input type="number" required min="1" placeholder={t('priceExample')} value={quoteForm.pricePerUnit}
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">{t('quoteQuantity', { unit: rfq.unit })}</label>
                  <input type="number" required min="1" step="any" placeholder={t('quantityExample')} value={quoteForm.quantity}
                    onChange={e => setQuoteForm({ ...quoteForm, quantity: e.target.value })}
                    className="w-full rounded-lg border dark:bg-white/5 dark:border-white/10 dark:text-gray-100 bg-white border-slate-200 text-slate-900 px-3 py-2 text-sm" />
                </div>
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">{t('notesOptional')}</label>
                <textarea rows={2} maxLength={500} placeholder={t('notesPlaceholder')} value={quoteForm.message}
              ...
              <button type="submit" disabled={submitting}
                ...
                {submitting ? t('submitting') : myQuote ? t('updateQuote') : t('submitQuote')}
              </button>
              {quoteMsg && <p className={`text-sm ${quoteMsg.includes(t('quoteSubmitted')) ? 'dark:text-green-400 text-green-600' : 'dark:text-red-400 text-red-600'}`}>{quoteMsg}</p>}
            </form>
          </div>
        )}
        ...
        {rfq.quotes.length > 0 && (
          <div className="mt-6 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold dark:text-gray-100 text-slate-900">{t('quoteList', { count: rfq.quotes.length })}</h2>
                ...
                <div key={q.id} className={`rounded-lg border dark:border-white/10 border-black/10 p-4 ${q.status === 'accepted' ? 'dark:bg-green-500/10 bg-green-50' : 'dark:bg-white/[0.02] bg-slate-50'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {q.status === 'accepted' && <span className="ml-2 rounded dark:bg-green-500/15 dark:text-green-400 bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">{t('selected')}</span>}
                      {q.status === 'declined' && <span className="ml-2 rounded dark:bg-red-500/15 dark:text-red-400 bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">{t('rejected')}</span>}
                    </div>
                  ...
                  {canSelectWinner && q.status === 'pending' && (
                    <button onClick={async () => {
                      if (!confirm(t('confirmSelectQuote'))) return;
                      ...
                      const data = await res.json();
                      if (res.ok) { alert(t('quoteSelected')); fetchRfq(); } else { alert(buildErrorMessage(data)); }
        ...
        {rfq.quotes.length === 0 && (
          <div className="mt-6 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-6 shadow-sm text-center">
            <p className="dark:text-[#8b9e93] text-slate-500">{t('noQuotesYet')}</p>
          </div>
        )}
      </main>
    </div>
