        ...
        if (search) {
          ...
          filtered = filtered.filter(
            (p: Product) =>
              ...
              p.farmer.email.toLowerCase().includes(q) ||
    ...
    <div className='min-h-screen dark:bg-[#0d1410] bg-slate-50'>
      ...
      <main className='mx-auto max-w-7xl px-6 py-8'>
        <Link
          ...
          className='mb-4 inline-block text-sm font-medium dark:text-green-400 text-green-600'
        >
        ...
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold dark:text-gray-100 text-slate-900'>
              {t('title')}
            </h1>
            ...
            <table className='w-full text-left text-sm'>
              <thead className='dark:border-white/10 dark:bg-white/5 dark:text-[#8b9e93] border-b bg-slate-100 text-xs text-slate-500'>
                <tr>
                  ...
                  <th className='px-4 py-3'>{t('tableHeaders.farmer')}</th>
                  <th className='px-4 py-3'>{t('tableHeaders.commodity')}</th>
                  <th className='px-4 py-3 text-right'>{t('tableHeaders.price')}</th>
                  <th className='px-4 py-3 text-center'>Grade</th>
                  <th className='px-4 py-3 text-right'>{t('tableHeaders.stock')}</th>
                  <th className='px-4 py-3 text-center'>{t('tableHeaders.status')}</th>
                  <th className='px-4 py-3 text-center'>{t('tableHeaders.actions')}</th>
                </tr>
              </thead>
              <tbody className='divide-y'>
                {products.map((p) => (
                  ...
                  >
                    <td className='px-4 py-3'>
                      <div className='flex items-center gap-3'>
                        {p.photos[0] ? (
                          ...
                          <div className='flex h-10 w-10 items-center justify-center rounded dark:bg-white/5 dark:text-gray-500 bg-gray-100 text-xs text-slate-400'>
                            ?
                          </div>
                        )}
                        <div>
                          <div className='font-medium dark:text-gray-100 text-slate-900 line-clamp-1'>
                            {p.title}
                          </div>
                          <div className='text-xs dark:text-gray-500 text-slate-400'>
                            {p.id.slice(0, 8)}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className='px-4 py-3'>
                      <div className='text-sm dark:text-gray-100 text-slate-900'>
                        {p.farmer.businessName || p.farmer.name}
                      </div>
                      <div className='text-sm dark:text-gray-500 text-slate-400'>
                        {p.farmer.email}
                      </div>
                    </td>
                    <td className='px-4 py-3 dark:text-[#8b9e93] text-slate-500'>
                      {p.commodity.name}
                    </td>
                    <td className='px-4 py-3 text-right font-medium dark:text-gray-100 text-slate-900'>
                    ...
                    <td className='px-4 py-3 text-center'>
                      <div className='flex items-center justify-center gap-2'>
                        <Link
                          ...
                          className='rounded dark:bg-blue-500/15 dark:text-blue-400 dark:hover:bg-blue-500/25 bg-blue-100 px-2 py-1 text-xs font-medium text-blue-700 hover:bg-blue-200'
                        >
                          {t('edit')}
                        </Link>
                        <button
        ...
        {totalPages > 1 && (
          <div className='mt-4 flex items-center justify-center gap-2'>
            ...
            <span className='text-sm dark:text-[#8b9e93] text-slate-500'>
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    aktif:
      'dark:bg-green-500/15 dark:text-green-400 bg-green-100 text-green-700',
    pre_order:
  ...
  return (
    <span
      className={`rounded px-2 py-0.5 text-xs font-medium ${styles[status] || styles.aktif}`}
    >
