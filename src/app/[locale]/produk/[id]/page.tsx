    ...
    <div className="min-h-screen dark:bg-[#0d1410] bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <div className="flex items-center gap-4 mb-4">
          <Link href="/produk" className="inline-block text-sm dark:text-green-400 text-green-600 hover:underline">&larr; {t("backToCatalog")}</Link>
          {user?.role === "admin" && (
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="mb-6 space-y-3 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-4">
              <InfoRow label={t("availableStock")} value={`${product.quantityAvailable.toLocaleString("id-ID")} ${product.unit}`} />
              <InfoRow label="Grade" value={`Grade ${product.grade}`} />
              {product.harvestDate && <InfoRow label={t("harvestEstimate")} value={new Date(product.harvestDate).toLocaleDateString("id-ID")} />}
              <InfoRow label={t("location")} value={`${product.latitude}, ${product.longitude}`} />
            </div>

            <h3 className="mb-2 font-semibold dark:text-gray-100 text-slate-900">{t("description")}</h3>
            <div className="mb-6">
            ...
            <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-4">
              <h3 className="mb-3 font-semibold dark:text-gray-100 text-slate-900">{t("seller")}</h3>
              <InfoRow label={t("name")} value={product.farmer.name} />
              <InfoRow label={t("business")} value={product.farmer.businessName || "-"} />
              <InfoRow label={t("address")} value={product.farmer.address} />
              <InfoRow label={t("phone")} value={product.farmer.phone} />
              <InfoRow label={t("verification")} value={product.farmer.verificationStatus === "verified" ? t("verified") : t("notVerified")} />
            </div>
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {chatLoading ? t("loading") : t("chatSeller")}
              </button>
              <button
