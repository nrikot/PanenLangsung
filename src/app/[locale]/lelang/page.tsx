...
function timeRemaining(endTime: string, status: string, t: (key: string) => string): string {
  ...
  if (hours > 0) return `${hours}${t('hoursShort')} ${mins}${t('minutesShort')}`;
          ...
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3'>
            ...
            <div>
              ...
              >
                ...
                <option value='berakhir'>{t('ended')}</option>
                <option value='selesai'>{t('completed')}</option>
              </select>
            </div>
            <div>
              <label className='mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700'>
        ...
        {loading ? (
          ...
          <>
            <p className='mb-4 text-sm dark:text-[#8b9e93] text-slate-500'>
              {t('showingAuctions', { count: auctions.length, total })}
            </p>
                ...
                >
                  <div className='mb-3 flex items-center justify-between'>
                    ...
                    {a.status === 'aktif' && (
                      <span className='rounded dark:bg-green-500/15 dark:text-green-400 bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700'>
                        {timeRemaining(a.endTime, a.status, t)}
                      </span>
                    )}
                  </div>
                  ...
                  <div className='mb-2 flex items-center gap-2'>
                    ...
                    <span className='rounded dark:bg-blue-500/15 dark:text-blue-400 bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700'>
                      {a.selectionMode === 'otomatis' ? t('automatic') : t('manual')}
                    </span>
                  </div>
