    ...
    <div className="min-h-screen dark:bg-[#0d1410] bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <Link href="/petani/dashboard" className="mb-2 inline-block text-sm dark:text-green-400 text-green-600 hover:underline">&larr; {tc("dashboard")}</Link>
            <h1 className="text-2xl font-bold dark:text-gray-100 text-slate-900">{t("manage")}</h1>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="dark:border-white/10 dark:bg-white/5 dark:text-[#8b9e93] border-b bg-slate-100 text-left text-sm text-slate-500">
                  <th className="px-4 py-3">{t("tableProduct")}</th>
                  <th className="px-4 py-3">{t("tableCommodity")}</th>
                  <th className="px-4 py-3">{t("tablePrice")}</th>
                  <th className="px-4 py-3">{t("tableGrade")}</th>
                  <th className="px-4 py-3">{t("tableStock")}</th>
                  ...
                  <tr key={p.id} className="dark:border-white/10 border-b last:border-b-0">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <Link href={`/petani/produk/${p.id}/edit`} className="text-sm dark:text-blue-400 text-blue-600 hover:underline">{t("edit")}</Link>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const statusKeys: Record<string, string> = {
    aktif: "statusAktif",
    pre_order: "statusPreOrder",
    habis: "statusHabis",
    nonaktif: "statusNonaktif",
  };
  return <span className={`rounded px-2 py-0.5 text-xs font-medium ${colors[status] || "dark:bg-white/5 dark:text-[#8b9e93] bg-gray-100 text-slate-500"}`}>{statusKeys[status] ? t(statusKeys[status]) : status.replace("_", " ")}</span>;
}
