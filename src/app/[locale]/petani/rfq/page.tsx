...
export default function RfqPetaniPage() {
  ...
  return (
    <DashboardLayout requiredRole="petani" title="RFQ" subtitle={t("viewRfq")} backHref="/petani/dashboard">
      <div className="mb-6 flex gap-2">
        {["open", "closed", ""].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            ...
            {s === "open" ? t("open") : s === "closed" ? t("closed") : t("allRfq")}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-12 text-center shadow-sm">
          <p className="dark:text-gray-400 text-slate-500">{t("noRfq")}</p>
        </div>
      ) : (
            ...
            <Link key={r.id} href={`/rfq/${r.id}`}
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="text-right text-xs dark:text-[#8b9e93] text-slate-500">
                  ...
                  <p>{t("detail.deadline")}: {formatDate(r.neededBy)}</p>
                  <p>{r._count.quotes} {t("bidder")}</p>
                </div>
              </div>
              <p className="mt-2 text-xs dark:text-[#8b9e93] text-slate-500">{t("locationLabel")}{r.deliveryLocation}</p>
            </Link>
          ))}
        </div>
      )}
