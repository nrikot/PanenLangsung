...
export default function LelangPublicPage() {
  ...
  useEffect(() => { fetchAuctions(); }, [page, filters.commodity_id, filters.status, filters.sort]);
  ...
  return (
    <div className="min-h-screen dark:bg-[#0d1410] bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <h1 className="mb-6 text-2xl font-bold dark:text-gray-100 text-slate-900">Lelang Hasil Panen</h1>
        ...
        {loading ? (
          <div className="py-20 text-center dark:text-[#8b9e93] text-slate-500">Memuat lelang...</div>
        ) : auctions.length === 0 ? (
          <div className="py-20 text-center dark:text-[#8b9e93] text-slate-500">Belum ada lelang ditemukan.</div>
        ) : (
          <>
            <p className="mb-4 text-sm dark:text-[#8b9e93] text-slate-500">Menampilkan {auctions.length} dari {total} lelang</p>
              ...
              {auctions.map(a => (
                <Link key={a.id} href={`/lelang/${a.id}`}
                  ...
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    {a.status === 'aktif' && (
                      <span className="rounded dark:bg-green-500/15 dark:text-green-400 bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">{timeRemaining(a.endTime, a.status)}</span>
                    )}
                  </div>
                  <h3 className="mb-2 text-base font-semibold dark:text-gray-100 text-slate-900 line-clamp-2 group-hover:text-green-400">{a.title}</h3>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className="rounded dark:bg-blue-500/15 dark:text-blue-400 bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">{a.selectionMode === 'otomatis' ? 'Otomatis' : 'Manual'}</span>
                  </div>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      <span className="dark:text-[#8b9e93] text-slate-500">Harga awal</span>
                      <span className="font-semibold dark:text-gray-100 text-slate-900">{formatRupiah(a.startPrice)}/{a.unit}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="dark:text-[#8b9e93] text-slate-500">Kuantitas</span>
                      <span className="dark:text-gray-100 text-slate-900">{a.quantity.toLocaleString('id-ID')} {a.unit}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="dark:text-[#8b9e93] text-slate-500">Penawaran</span>
                      <span className="dark:text-gray-100 text-slate-900">{a._count.bids}</span>
                    </div>
                  </div>
                  <div className="mt-3 flex items-center justify-between border-t dark:border-white/10 border-black/10 pt-3">
                    <span className="text-xs dark:text-[#8b9e93] text-slate-500">{a.farmer.businessName || a.farmer.name}</span>
                    <span className="text-xs dark:text-[#8b9e93] text-slate-500">{formatDate(a.endTime)}</span>
                  </div>
                </Link>
              ))}
            </div>
              ...
              <Pagination className="mt-8">
                <PaginationContent>
                  ...
                  {generatePageNumbers(page, totalPages).map((p, i) =>
                    p === 'ellipsis' ? <PaginationItem key={`e-${i}`}><PaginationEllipsis /></PaginationItem> :
