...
export default function AdminPembeliPage() {
  ...
  return (
    <DashboardLayout requiredRole="admin" title="Daftar Pembeli" subtitle="Kelola semua akun pembeli" backHref="/admin/dashboard">
      ...
      <p className="mb-4 text-sm dark:text-[#8b9e93] text-slate-500">Total: {total} pembeli</p>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-12 text-center shadow-sm">
          <p className="dark:text-gray-400 text-slate-500">Tidak ada pembeli ditemukan.</p>
        </div>
      ) : (
        <>
                  ...
                  <tr key={b.id} className="dark:border-white/5 border-b border-gray-100 hover:dark:bg-white/[0.02] hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[b.verificationStatus] || ""}`}>
                        {b.verificationStatus === "verified" ? "Terverifikasi" : b.verificationStatus === "rejected" ? "Ditolak" : "Menunggu"}
                      </span>
                    </td>
