...
export default function AdminPetaniPage() {
  ...
  return (
    <DashboardLayout requiredRole="admin" title="Daftar Petani" subtitle="Kelola semua akun petani" backHref="/admin/dashboard">
      <div className="mb-6 flex flex-wrap items-end gap-4">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          />
          <button type="submit" className="rounded-lg bg-green-700 px-4 py-2 text-sm font-medium text-white hover:bg-green-800">Cari</button>
        </form>
        <div className="flex gap-2">
          {[
            { val: "", label: "Semua" },
            { val: "verified", label: "Terverifikasi" },
      ...
      <p className="mb-4 text-sm dark:text-[#8b9e93] text-slate-500">Total: {total} petani</p>

      {loading ? (
        <div className="py-12 text-center dark:text-[#8b9e93] text-slate-500">Memuat data...</div>
      ) : farmers.length === 0 ? (
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-12 text-center shadow-sm">
          <p className="dark:text-gray-400 text-slate-500">Tidak ada petani ditemukan.</p>
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="dark:border-white/10 dark:bg-white/5 dark:text-[#8b9e93] border-b bg-slate-100 text-xs text-slate-500">
                  ...
                  <th className="px-4 py-3 font-medium">Usaha</th>
                  <th className="px-4 py-3 font-medium">Verifikasi</th>
                  <th className="px-4 py-3 font-medium">Produk</th>
                  <th className="px-4 py-3 font-medium">Lelang</th>
                  ...
                  <tr key={f.id} className="dark:border-white/5 border-b border-gray-100 hover:dark:bg-white/[0.02] hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[f.verificationStatus] || ""}`}>
                        {f.verificationStatus === "verified" ? "Terverifikasi" : f.verificationStatus === "rejected" ? "Ditolak" : "Menunggu"}
                      </span>
                    </td>
