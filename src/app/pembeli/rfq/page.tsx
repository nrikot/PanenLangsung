...
interface Rfq {
  ...
  commodity: { id: string; name: string }; _count: { quotes: number };
}
...
export default function RfqPembeliPage() {
  ...
  const [form, setForm] = useState({ commodityId: "", quantity: "", unit: "kg", targetPrice: "", deliveryLocation: "", neededBy: "" });
  ...
  const fetchRfqs = useCallback(async () => {
    ...
    setRfqs((data.rfqs || []).filter((r: Rfq) => true));
    setLoading(false);
  }, [user]);
    ...
    try {
      const res = await fetch("/api/v1/rfqs", {
        ...
        body: JSON.stringify({
          ...
          deliveryLocation: form.deliveryLocation, neededBy: form.neededBy,
        }),
      });
      const data = await res.json();
      ...
      setShowCreate(false);
      setForm({ commodityId: "", quantity: "", unit: "kg", targetPrice: "", deliveryLocation: "", neededBy: "" });
      fetchRfqs();
    } catch (e: unknown) { setFormMsg(e instanceof Error ? e.message : "Gagal membuat RFQ"); }
    finally { setSubmitting(false); }
  }

  async function cancelRfq(id: string) {
    if (!confirm("Batalkan RFQ ini?")) return;
    const res = await fetch(`/api/v1/rfqs/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "cancelled" }) });
    const data = await res.json();
    if (res.ok) fetchRfqs(); else alert(data.error);
  }

  return (
    <DashboardLayout requiredRole="pembeli" title="RFQ" subtitle="Buat & kelola permintaan penawaran" backHref="/pembeli/dashboard">
      <div className="mb-6">
      ...
      {showCreate && (
        <div className="mb-8 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-bold dark:text-gray-100 text-slate-900">Buat RFQ Baru</h2>
          <form onSubmit={createRfq} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <input type="number" required min="1" step="any" placeholder="Contoh: 1000" value={form.quantity}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">Target Harga (Rp, opsional)</label>
                <input type="number" min="1" placeholder="Contoh: 12000" value={form.targetPrice}
                  onChange={e => setForm({ ...form, targetPrice: e.target.value })}
                  className="w-full rounded-lg border dark:bg-white/5 dark:border-white/10 dark:text-gray-100 bg-white border-slate-200 text-slate-900 px-3 py-2 text-sm" />
              </div>
              <div className="sm:col-span-2">
                <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">Lokasi Pengiriman</label>
                <input required minLength={5} maxLength={500} placeholder="Contoh: Jl. Merdeka No. 10, Jakarta" value={form.deliveryLocation}
                  onChange={e => setForm({ ...form, deliveryLocation: e.target.value })}
                  className="w-full rounded-lg border dark:bg-white/5 dark:border-white/10 dark:text-gray-100 bg-white border-slate-200 text-slate-900 px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium dark:text-gray-300 text-slate-700">Kebutuhan Sebelum</label>
                <input type="date" required value={form.neededBy}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-12 text-center shadow-sm">
          <p className="dark:text-gray-400 text-slate-500">Anda belum memiliki RFQ.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {rfqs.map(r => (
            <div key={r.id} className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm dark:text-gray-100 text-slate-900">{r.quantity.toLocaleString("id-ID")} {r.unit}{r.targetPrice ? ` — Target ${formatRupiah(r.targetPrice)}` : ""}</p>
                  <p className="text-xs dark:text-[#8b9e93] text-slate-500 mt-1">Lokasi: {r.deliveryLocation} — Kebutuhan: {formatDate(r.neededBy)}</p>
                </div>
                <div className="flex gap-2">
                  <Link href={`/rfq/${r.id}`} className="rounded-lg dark:bg-white/5 dark:text-gray-300 dark:hover:bg-white/10 bg-slate-100 text-slate-700 hover:bg-slate-200 px-4 py-1.5 text-xs font-medium">Lihat</Link>
                  {r.status === "open" && (
                    <button onClick={() => cancelRfq(r.id)} className="rounded-lg dark:bg-red-500/15 dark:text-red-400 bg-red-100 text-red-700 hover:bg-red-200 px-4 py-1.5 text-xs font-medium">Batalkan</button>
                  )}
                </div>
              </div>
              <div className="mt-2 text-xs dark:text-[#8b9e93] text-slate-500">{r._count.quotes} penawaran masuk</div>
            </div>
          ))}
        </div>
      )}
