...
export default function LelangPembeliPage() {
  ...
  return (
    <DashboardLayout requiredRole="pembeli" title="Lelang" subtitle="Temukan & ikuti lelang dari petani" backHref="/pembeli/dashboard">
      <div className="mb-6 flex gap-2">
        {["aktif", "berakhir", "selesai", ""].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            ...
            {s === "aktif" ? "Aktif" : s === "berakhir" ? "Berakhir" : s === "selesai" ? "Selesai" : "Semua"}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-12 text-center shadow-sm">
          <p className="dark:text-gray-400 text-slate-500">Tidak ada lelang ditemukan.</p>
        </div>
      ) : (
          ...
          {auctions.map(a => (
            <Link key={a.id} href={`/lelang/${a.id}`}
              ...
              <div className="mt-3 flex gap-4 text-sm">
                ...
                <span className="dark:text-[#8b9e93] text-slate-500">{a._count.bids} penawaran</span>
              </div>
            </Link>
          ))}
        </div>
