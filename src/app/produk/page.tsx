...
export default function ProdukPage() {
  ...
  const fetchNearby = useCallback(
    async (loc?: { lat: number; lng: number }) => {
      ...
      if (!pos) {
        alert('Lokasi GPS tidak tersedia');
        return;
      }
      setLoading(true);
      const params = new URLSearchParams({
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1 && attempt === 0) {
          ...
          if (err.code === 1) {
            alert(
              "Izin lokasi ditolak. Pastikan izin lokasi aktif di browser DAN di sistem operasi (Windows: Settings > Privacy > Location > 'Let desktop apps access your location').",
            );
          } else if (err.code === 2) {
  ...
  function requestLocationAndSearch() {
    ...
    if (!window.isSecureContext) {
      alert(
        'Geolokasi membutuhkan HTTPS. Akses situs melalui HTTPS atau localhost.',
      );
      return;
    }
      ...
      <main className='mx-auto max-w-7xl px-6 py-8'>
        ...
        <div className='mb-8 rounded-xl border dark:bg-white/[0.03] dark:border-white/10 bg-white border-black/10 p-4 shadow-sm'>
          <div className='mb-4 flex gap-2'>
            ...
            >
              {geoLoading ? 'Mencari lokasi...' : 'Terdekat dari Saya'}
            </button>
          </div>
        ...
        {loading ? (
          ...
          <>
            <p className='mb-4 text-sm dark:text-[#8b9e93] text-slate-500'>
              {mode === 'nearby'
                ? `Menampilkan ${products.length} produk terdekat`
                ...
                >
                  ...
                  <div className='p-4'>
                    ...
                    <p className='text-xs dark:text-[#8b9e93] text-slate-500'>
                      Stok: {p.quantityAvailable.toLocaleString('id-ID')}{' '}
                      {p.unit}
                    </p>
                    {p.distance_km !== undefined && (
                      <p className='mt-1 text-xs font-medium text-orange-600'>
                        {p.distance_km} km dari Anda
                      </p>
                    )}
